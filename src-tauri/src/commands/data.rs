...
pub async fn export_all_data(state: State<'_, AppState>) -> Result<AppDataExport, String> {
    ...
    let base_resumes = stmt.query_map([], |row| {
        Ok(ResumeDetail {
            id: row.get(0)?,
            name: row.get(1)?,
            category: row.get(2)?,
    ...
    let tailored_resumes = stmt.query_map([], |row| {
    ...
    let compiler_state: Option<String> = conn.query_row(
...
pub async fn import_data(
    ...
    if mode == "overwrite" {
        ...
        tx.execute("DELETE FROM tailored_resumes", []).map_err(|e| e.to_string())?;
        tx.execute("DELETE FROM jobs", []).map_err(|e| e.to_string())?;
        tx.execute("DELETE FROM base_resumes", []).map_err(|e| e.to_string())?;
        tx.execute("DELETE FROM compiler_state", []).map_err(|e| e.to_string())?;
    }
    ...
    for job in data.jobs {
        tx.execute(
            ...
            [
                ...
                &job.created_at.unwrap_or_else(|| chrono::Local::now().to_rfc3339()),
                &job.updated_at.unwrap_or_else(|| chrono::Local::now().to_rfc3339()),
            ],
        ).map_err(|e| e.to_string())?;
    }
